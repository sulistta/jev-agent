...
function AssistantMessageCard({ event }: { event: AssistantMessageEvent }) {
	const label =
		event.purpose === 'clarification'
			? 'Question'
			: event.purpose === 'summary'
				? 'Summary'
				: 'Assistant'
	return (
		<div className="rounded-lg border-l-2 border-l-violet-500/60 border bg-violet-500/5 p-2.5">
			<div className="text-[10px] font-medium text-violet-500 mb-1">{label}</div>
			<p className="text-xs text-foreground whitespace-pre-wrap">{event.text}</p>
		</div>
	)
}
...
export function EventCard({ event }: { event: HistoricalEvent }) {
	...
	if (event.type === 'assistant_message') return <AssistantMessageCard event={event} />
