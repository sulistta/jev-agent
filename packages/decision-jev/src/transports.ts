...
function formatHttpError(endpoint: string, status: number, details?: string): string {
	...
	const contextHint =
		status === 413 || status === 422
			? ' The provider rejected the complete candidate context; no candidates were truncated.'
			: ''
	return `Jev HTTP ${status}.${contextHint}${details ? ` Provider response: ${details}` : ''}`
}
